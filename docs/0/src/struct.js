...
const getDefaultValue = (v) => {
    /*
    if (isT.p===v || isT.p.bln===v) return false;
    if (isT.p.int || isT.p.fin) return 0;
    if (isT.p.big===v) return 0n;
    if (isT.p.str===v) return '';
    if (isT.p.sym===v) return Symbol();
    if (isT.o.ary===v) return [];
    if (isT.o.obj===v) return {};
    return null;
    */
