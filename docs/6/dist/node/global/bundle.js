(() => {
  // ../../../../typ-build-5694ocxi684/wrapped.js
  (function() {
    var module = { exports: {} };
...
Actual: ${Tys.name(v)}`);
    ...
    var getDefaultValue = (validator) => isT.o.obj(validator) ? makeBaseObj2(validator) : dfv(validator);
    var makeBaseObj2 = (o) => {
      const base = {};
    ...
    var createDataSetter = (baseData, name, validator) => (v) => {
      if (validator(v)) {
        baseData[name] = v;
      }
    };
    ...
    var createNormalUndefinedHandler = (baseData, name, validator, nestedStruct, mode) => (k, val) => {
      baseData[name][k] = val;
      validator[k] = () => true;
      Object.defineProperty(nestedStruct, k, makeDescriptor(baseData[name], nestedStruct, k, validator[k], mode));
    };
    var createNestedSetter = (validator, undefinedHandler) => (v, nestedStruct) => {
      owT.o.obj(v);
    ...
    var createSetter = (baseData, name, structObj, validator, mode) => {
      if (mode === "freeze") {
        return createFreezeSetter();
      }
      if (isT.o.obj(validator)) {
        const nestedStruct = structObj[name];
        const undefinedHandler = mode === "normal" ? createNormalUndefinedHandler(baseData, name, validator, nestedStruct, mode) : createSealUndefinedHandler;
        return createNestedSetter(validator, undefinedHandler);
      }
      return createDataSetter(baseData, name, validator);
    };
    var createGetter = (baseData, name, validator, nestedStruct) => {
      if (isT.o.obj(validator)) {
        return () => nestedStruct;
      }
    ...
    var createStruct = (o, existingBase = null, mode = "normal") => {
      owT.o.obj(o);
      const base = { _: existingBase || makeBaseObj2(o) };
      const structObj = {};
    ...
    var struct = (o) => createStruct(o, null, "normal");
