...
function assertThrow(Err, msg, fn) {
    ...
    if (null===err) throw new Error(`例外発生すべきテスト項目で発生しませんでした。`);
//    expect(err).not.toBeNull();
    expect(err).toBeInstanceOf(Err);
//    expect(err.message).toBe(msg);
    if ('string'===typeof msg) expect(err.message).toBe(msg);
    if (msg instanceof RegExp) expect(err.message).toMatch(msg);
}
// テストデータ
class C {
    static get sg() {}
